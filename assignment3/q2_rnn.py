...
class RNNModel(NERModel):
    ...
    def add_prediction_op(self):
        ...
        with tf.variable_scope("RNN", reuse=tf.AUTO_REUSE):
            U = tf.get_variable("U", shape=[Config.hidden_size, Config.n_classes],initializer=tf.contrib.layers.xavier_initializer(), trainable=True)
            b2 = tf.get_variable("b2", shape=[Config.n_classes],initializer=tf.constant_initializer(0), trainable=True)
